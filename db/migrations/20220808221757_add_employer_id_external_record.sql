...
ALTER TABLE eligibility.file_parse_results
    ADD employer_assigned_id eligibility.iwstext;
...
CREATE FUNCTION eligibility.merge_file_parse_results_members(files bigint[], show_missing boolean) RETURNS TABLE(id bigint, organization_id bigint, first_name eligibility.iwstext,
    ...
    do_not_contact eligibility.iwstext, gender_code eligibility.iwstext, employer_assigned_id eligibility.iwstext,
...
CREATE VIEW eligibility.file_parse_results_members_merged AS
 SELECT merge_file_parse_results_members.*
   FROM eligibility.merge_file_parse_results_members(( SELECT array_agg(( SELECT DISTINCT file_parse_results.file_id
                   FROM eligibility.file_parse_results)) AS array_agg), true) merge_file_parse_results_members(id, organization_id, first_name, last_name, email, unique_corp_id, dependent_id, date_of_birth, work_state, gender_code, do_not_contact, employer_assigned_id, record, file_id, effective_range, errors, warnings, created_at, updated_at, is_missing);
